...
class Player
{
  constructor ( params )
  {
    ...
    for ( let key in params.player.input )
    {
      if ( !isNaN( key ) )
      { this.actions[params.player.input[key].action] = { "active":false }; }
      else
      { this.actions[key] = { "active":{ "x":0, "y":0 } }; }
    }


    postMessage( { "to":params.input, "from":"engine", "command":"generatePlayer", "message":{ "name":this.name, "input":params.player.input } } );
  }
